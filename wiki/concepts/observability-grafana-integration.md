...
- [op-reth --max-outbound-peers & devp2p 피어 슬롯 제어](op-reth-max-outbound-peers.md) — 메트릭 축에서 `network_outgoing_connections` / `network_pending_outgoing_connections` / `network_outbound_*` 카운터로 피어 슬롯 설정이 실제로 적용됐는지, 병목이 내 설정인지 상대 노드 포화인지 판별한다.
