...
- [op-reth "HTTP request to sequencer failed ... -32003: txpool is full" 진단](op-reth-sequencer-forward-txpool-full.md) — 같은 op-reth 로그 진단 계열이지만 **성격이 반대**다. 본 페이지의 WARN은 무해·자가복구인 반면, 그쪽은 사용자 tx가 실제로 유실되는 실패 신호이므로 원인 제거가 필요하다.
