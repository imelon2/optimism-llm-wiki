...
- [OP Stack 트랜잭션 전파 경로 (Ingress → 시퀀서)](concepts/op-stack-tx-ingress-propagation.md) — 사용자 tx가 시퀀서에 닿는 두 경로(HTTP 포워딩 vs devp2p gossip)와 Optimism 공식 Tx Ingress 아키텍처, op-geth는 포워딩 시 로컬 풀 미보존이 기본이나 op-reth는 항상 보존하는 차이, Ingress가 덜어주는 것(읽기 RPC·무효 트래픽·중복·장애 격리)과 못 덜어주는 것(ecrecover·nonce/잔액 검증은 시퀀서가 전부 재수행), 조용한 장애 3종(synced=false·풀 포화·nonce 갭)과 op-conductor가 tx 제출을 프록시하지 않는다는 사실
- [트랜잭션 √n 브로드캐스트 규칙 (원본 전송 vs 해시 알림)](concepts/tx-propagation-sqrt-broadcast.md) — 새 tx 원본을 피어 일부(약 √n)에게만 보내고 나머지엔 해시만 알리는 규칙의 지연 비용이 클라이언트마다 500배 차이(op-reth는 즉시 요청 ~1 RTT, op-geth는 txArriveTimeout 400ms 대기), 선택 기준도 피어 순번(reth, off-by-one으로 round(√n)+1개) vs sender 주소(geth, ceil(√n))라 증상이 "전부 느리거나 전부 빠름" vs "특정 지갑만 느림"으로 반대, 알림은 원본이 먼저 오면 무해하게 버려짐, devp2p 표준이 아니며 --tx-propagation-mode/-policy 설정과 fetched_transactions 계열 측정법
- [txpool nonce 갭으로 인한 트랜잭션 전파 정지](concepts/txpool-nonce-gap-propagation-stall.md) — LB가 한 계정의 연속 nonce를 여러 Ingress에 흩뿌리면 갭에 막힌 tx가 queued에 갇혀 gossip 자체가 안 되고 블록당 한 칸씩만 전진(6개가 4블록 8초), 풀이 갭을 명시적으로 허용해(FirstNonceGap: nil) 에러·로그·메트릭이 전혀 없는 무증상 장애, 전파 이벤트가 pending 승격 시에만 발행되는 것이 원인, sticky LB(source IP)와 Ingress 상호 피어링으로 해소
