...
`bridge`, `fault-proofs`, `derivation`, `batcher`, `proposer`, `sequencer`, `withdrawals`, `deposits`, `governance`, `da`(data availability), `p2p`(networking), `reth`(EL client), `txpool`(mempool/tx pool), `l1`, `l2`, `rfc`, `observability`, `monitoring`, `sre`.
